...
class HistoryIndex {
  ...
  Map<String, dynamic> toJson() =>
      {'time': time, 'gids': gids?.map((e) => e.toJson()).toList()};

  HistoryIndex clone() =>
      HistoryIndex(time: time, gids: gids?.map((e) => e.clone()).toList());

  HistoryIndex copyWith({int? time, List<HistoryIndexGid>? gids}) =>
      HistoryIndex(
        time: time ?? this.time,
        gids: gids ?? this.gids,
      );
